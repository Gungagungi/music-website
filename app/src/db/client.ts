...
function connectionString(): string {
  ...
  if (!url) {
    throw new Error(
      'DATABASE_URL is missing. Run `npm run db:up`, then copy .env.example to .env.',
    );
  }

  // The driver parses this string as a URL and, on failure, throws an
  // "Invalid URL" that hides its input — out of caution, since it contains a
  // password. The message then points at nothing at all. The cause is almost
  // always the same: a password generated in base64, whose `/` ends the
  // authority section.
  try {
    new URL(url);
  } catch {
    throw new Error(
      'DATABASE_URL is malformed. Most common cause: a `/`, `@` or `?` character ' +
        'in the password, which cuts the URL. Generate a password without one: openssl rand -hex 32.',
    );
  }
