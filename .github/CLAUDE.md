...
- **`baseline-perf.yml`** — mesure les seuils k6 sur le runner et recommite `perf/baseline.json`. Même principe que les baselines visuelles : la référence appartient à l'environnement qui la compare. `K6_CALIBRATION=1` désactive les seuils le temps de la mesure, sinon le run échouerait sur ceux qu'il sert à produire. À relancer quand la machine change (image du runner, version de PostgreSQL), **pas** à chaque changement de code : une baseline reprise à chaque commit entérine la lenteur qu'elle devrait détecter.
- **`pages.yml`** — publie le rapport sur https://gungagungi.github.io/music-website/, tous les jours à 02:30 UTC et à la demande. Il relance sa propre suite au lieu de réutiliser le rapport du nightly : dépendre d'un artifact produit ailleurs ferait échouer la publication chaque fois que le nightly échoue, alors qu'un rapport rouge est exactement ce qu'on veut publier. La documentation de `docs/` est rendue en HTML par `scripts/rendre-docs.mjs` au lieu d'être copiée en Markdown brut ; le script échoue sur un lien interne mort plutôt que de publier une page qui promet ce qu'elle n'a pas. Corollaire : ses étapes de suite sont en `continue-on-error`, sans quoi le workflow s'arrêterait avant de publier précisément le jour où le rapport aurait le plus à dire.
...
- k6 ne matérialise une sous-métrique taguée (`group_duration{group:::Catalogue}`) que si un seuil la nomme. En mode calibration, où les seuils sont retirés, il faut donc lui en laisser un — même absurde — sans quoi la métrique est absente du résumé et il n'y a rien à enregistrer.
